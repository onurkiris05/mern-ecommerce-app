...
function NewProductPage() {
  ...
  async function onSubmit(input: ProductApi.ProductInput) {
    ...
    try {
      const response: any = await CloudinaryApi.uploadImage(imageFile!);
      ...
      try {
        imageUrl && (await CloudinaryApi.deleteImage(imageUrl));
      } catch (error) {
        console.error(error);
      }
  ...
  return (
    <Container>
      {isLoading ? (
        <LoadIndicator message="Creating..." />
      ) : (
        <Wrapper>
          ...
          <Form className="d-flex flex-column" onSubmit={handleSubmit(onSubmit)}>
            ...
            <Form.Group className="mb-4">
              <Form.Label className="me-4">Image: </Form.Label>
              <ImageForm onChange={(file) => setImageFile(file)} />
            </Form.Group>
            <Form.Group className="mb-4">
